import { Event, TicketTypeConfig, Vendor, User, Refund } from '@/types/schema';
import {
  dummyEvents,
  dummyTicketTypes,
  dummyVendors,
...
// Define interfaces based on your actual data structures
interface Attendee {
  id: string;
  name: string;
  email: string;
  ...
  checkedIn: boolean;
  eventId?: string;
  // Add other properties that exist in your dummyAttendees
}

// Use a more flexible AnalyticsData type that matches your actual dummy data
type AnalyticsData = (typeof dummyAnalytics)[0]['data'];
...
export const api = {
  ...
  getAttendees: async (eventId?: string): Promise<Attendee[]> => {
    await delay(500);
    if (eventId) {
      const ticketIds = dummyTicketTypes
        .filter((ticket) => ticket.eventId === eventId)
        .map((ticket) => ticket.id);
      return dummyAttendees.filter((attendee) =>
        ticketIds.includes(attendee.ticketId)
      );
    }
  ...
  checkInAttendee: async (attendeeId: string): Promise<void> => {
    await delay(500);
    const index = dummyAttendees.findIndex(
  ...
  getRefundRequests: async (eventId?: string): Promise<Refund[]> => {
    await delay(500);
    if (eventId) {
      const ticketIds = dummyTicketTypes
        .filter((ticket) => ticket.eventId === eventId)
        .map((ticket) => ticket.id);
      return dummyRefunds.filter((refund) =>
        ticketIds.includes(refund.ticketId)
      );
    }
    return dummyRefunds;
  },
